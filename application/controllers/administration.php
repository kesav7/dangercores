...
class Administration extends MY_Controller {
	...
	public function create_user( $type = '' )
	{
		...
		if( $this->require_group('employees') )
		{
			...
			if( ! empty( $type ) )
			{
				...
				$data = array(
				'view_file' => 'administration/create_user',
				'user_creation_form' => $view_data['user_creation_form'],
	...
	public function manage_users( $page = 1 )
	{
		...
		if( $this->require_role('admin,manager') )
		{
			...
			else
			{
				...
				$data_datas = array(
					'view_file' => $this->load->view('administration/manage_users', $view_data, TRUE)
				);
				...
				$data = array(
					...
					'view_file' => 'administration/manage_users',
					
				);

				$this->front_template->load_template($data,$data_datas);
			}
		}
	...
	public function deny_access()
	{
		...
		if( $this->require_role('admin') )
		{
			...
			$data_datas = array(
				'view_file' => $this->load->view( 'administration/deny_access', ( isset( $view_data ) ) ? $view_data : '', TRUE ),
			);
			$data = array(
				'javascripts' => array(
					'js/jquery.char-limiter-3.0.0.js',
				...
				'view_file' => 'administration/deny_access',
			);
